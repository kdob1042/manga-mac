...
8. コマの画像をドラッグして修正可能領域を指定し、自然言語で指示します。範囲外のRGBA画素は元画像から復元します。
9. 人物・原作整合を確認し、PNG/CBZへ出力します。文字が入り切らない場合は省略せずエラーにします。
...
- mainのSHAを一度取得し、全ファイルを同じ40桁SHAで取得。同期取得失敗は現行スナップショットに影響しません。新版取得と適用は別操作。
- 原文の段落IDをLLMへ渡し、全IDの順序・一意性・完全性を検証。表示原文はスナップショットから直接取得。AI出力で台詞を上書きしません。
- キャラの画像バイト列をSHA256で記録し、Rustでも照合してからSwiftのmoodboard入力へ渡します。入力順、人物ID、ハッシュをコマに記録します。見た目の一致は保証しません。
...
- 自動人物検出、SAMマスク、顔の同一性検査と自動修復は未実装。現状は矩形選択と人による見た目確認です。
- コマ内の自動吹き出し配置・日本語縦書き/Core Textは未実装。原文は画像下の独立文字層として保持します。地の文も省略しないため、長い段落は組版調整が必要です。
- 原作変更時は変更場面（設定変更時は対象の全場面）の再計画。意味差分による台詞のみ更新、矛盾しない演出修正の自動引継ぎ、低頻度自動同期、ETagは未実装。旧画像と履歴はスナップショットに紐づき保持されます。
