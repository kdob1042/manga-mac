...
候補を比較して採用するとPNG/CBZにも反映。元画像は不変の作画履歴に残り、通常の「元に戻す」で復元できます。配置と文字は変更せず、候補作成後に配置が変わると採用を拒否します。途中終了時は自動再実行せず、要求を取り下げてから再作成してください。AI超解像・細部復元とMac品質評価はIssue #57の未完項目です。
