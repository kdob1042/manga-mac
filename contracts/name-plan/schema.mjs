...
export function validatePlan(plan, atoms, characterIds, contextAtoms = atoms) {
  ...
  return { coverage, atomMap };
      ...
      scenes: arr(obj({ id, sha256: { type: 'string', pattern: '^[0-9a-f]{64}
    policyVersion: str(100), provenance: obj({ producer: str(100), model: { type: 'string', maxLength: 200 }, editedBy: arr(str(100), 20) }),
    plan: planSchema,
  }), $defs: { tree },
};
...
export function validatePlan(plan, atoms, characterIds, contextAtoms = atoms) {
 ...
 } }), 1000, 1), selectedAtomIds: ids(),
      settingsHash: { type: 'string', pattern: '^[0-9a-f]{64}
    policyVersion: str(100), provenance: obj({ producer: str(100), model: { type: 'string', maxLength: 200 }, editedBy: arr(str(100), 20) }),
    plan: planSchema,
  }), $defs: { tree },
};
...
export function validatePlan(plan, atoms, characterIds, contextAtoms = atoms) {
 ...
 }, referencesHash: { type: 'string', pattern: '^[0-9a-f]{64}
    policyVersion: str(100), provenance: obj({ producer: str(100), model: { type: 'string', maxLength: 200 }, editedBy: arr(str(100), 20) }),
    plan: planSchema,
  }), $defs: { tree },
};
