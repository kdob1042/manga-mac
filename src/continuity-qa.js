...
export async function runContinuityQA({project,panelId,imageCapable=false,ask}) {
  ...
  const panel=project.panels.find(p=>p.id===panelId),previous=project.panels.find(p=>p.id===panel?.continuity?.previousPanelId);
  ...
  const raw=await ask({purpose:'vision',images:[previous.image,panel.image],schema,prompt:JSON.stringify({task:'2枚の漫画コマ画像を読書順に比較し、衣装・持ち物・汗/傷・人物の位置/表情が不自然に飛んでいないか指摘する。原稿や描かれていない設定を推測して事実扱いしない。変更を実行せず、問題がなければfindings:[]を返す。',previousPanelId:previous.id,panelId:panel.id,expected:panel.continuity,visualOnly:true})});
...
export async function assertContinuityQACurrent(project,qa) {
  ...
  if(!panel?.image||!previous?.image||panel.continuity?.previousPanelId!==previous.id||panel.sceneId!==previous.sceneId||qa.base!==await nameReadToken(project)||qa.panelHash!==await imageHash(panel.image)||qa.previousHash!==await imageHash(previous.image))throw Error('画像間QA中にコマが変わりました');
}
