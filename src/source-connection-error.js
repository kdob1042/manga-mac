...
export function sourceConnectionError(error, context = {}) {
  ...
  const kind = unsupported ? 'unsupported' : limit ? 'limit' : auth ? 'auth' : missing ? 'missing' : network ? 'network' : 'unknown';
  const guidance = {
  ...
  if (/Unsupported GitHub operation/.test(message)) technical = 'Unsupported GitHub operation';
