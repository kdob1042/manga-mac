import { initialLayout, ensureLayout, validateLayout, layoutWarnings, pagePanels, contentBox, bounds, PAGE } from './layout.js';
...
export async function pageLayers(panels, snapshots, localizations = [], locale = 'ja', layer = 'complete', page = null, draft = false, imageCrops = {}) {
  ...
  for (const slot of page.slots) {
    ...
    if(crop && p.image && layer!=='overlay') {
      const im=await imageOf(p.image), b=bounds(slot.points);
      const r=cropRect(im.width,im.height,{x:b.x*PAGE.width,y:b.y*PAGE.height,width:b.width*PAGE.width,height:b.height*PAGE.height},crop);
    ...
    if(crop && layer==='complete') {
      ctx.beginPath();slot.points.forEach(([x,y],i)=>i?ctx.lineTo(x*PAGE.width,y*PAGE.height):ctx.moveTo(x*PAGE.width,y*PAGE.height));ctx.closePath();ctx.strokeStyle='#111';ctx.lineWidth=4;ctx.stroke();
    }
  }
  return canvas.toDataURL('image/png');
}
export const pagePNG = (panels, snapshots, localizations = [], locale = 'ja', page = null, draft = false, imageCrops = {}) => pageLayers(panels, snapshots, localizations, locale, 'complete', page, draft, imageCrops);
...
export async function exportCBZ(project) {
  ...
  const zip = new JSZip();
  for (const [i,page] of project.layout.pages.entries()) zip.file(`${String(i + 1).padStart(3, '0')}.png`, (await pagePNG(pagePanels(project,page), project.snapshots, project.localizations, project.output_locale,page,false,project.layout.imageCrops)).split(',')[1], { base64: true });
