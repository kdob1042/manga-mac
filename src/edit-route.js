// Only known operations cross the Blender boundary; never execute generated code.
export function editRoute(instruction) {
  ...
  if (/カメラ|焦点距離|構図/.test(instruction)) {
  ...
  if (/ポーズ|二人.*(距離|近|離)|立ち位置|体の向き/.test(instruction)) return { kind: 'blender' };
  return { kind: 'region' };
}
