...
export default function BlenderSettings({ disabled, run, notify, project, current, commit }) {
  ...
  return <fieldset disabled={disabled}><legend>Blender 4.5.13 · GUI接続</legend>
    ...
    <TripoSettings project={project} current={current} commit={commit} disabled={disabled} run={run} notify={notify}/>
    ...
    <p>素材の追加・Asset Browser・ポーズの選択は、接続したBlenderの画面で行えます。保存済みの撮影と採用履歴は保持しています。各コマの詳細調整で「このコマのBlenderを開く」を押すと、作業用GUIの起動と接続をアプリが行います。</p>
  </fieldset>;
}
