...
export default function LayoutEditor({project,current,commit,run,busy,pageIndex,setPage,model,selected,cancelled}) {
  ...
  const [imageMode,setImageMode]=useState(false),[dimensions,setDimensions]=useState({}),[fullVideo,setFullVideo]=useState(null);
  const layout=draft??project.layout,page=layout?.pages[pageIndex];
  const motionSources=useMotionSources(project,page?pagePanels(project,page):[]);
  ...
  return <section className="layout-editor" aria-label="コマ割り編集">
    ...
    <p>コマを選び、四隅をドラッグして変形。枠内をドラッグすると全体を移動します。Escで取消。絵や本文は変更しません。枠破りでは、従来のコマ割りを描いたうえに、ホーム枠の外へ出る作画だけが隣の上に乗ります。破線ははみ出し範囲です。割当済みのコマ動画はホーム枠でマスクして再生します。</p>
    ...
    {page&&<div className="layout-scroll"><div className="layout-stage" style={{width:`${zoom}%`}}><svg ref={svg} className="layout-canvas" viewBox="0 0 1600 2260" onPointerMove={move} onPointerUp={end} onPointerCancel={cancelDrag} onLostPointerCapture={()=>{if(gesture.current)cancelDrag();}}>
    ...
    <div className="layout-videos">{page.slots.map(s=>{const play=s.panelId&&motionSources[s.panelId];return play?.src?<PanelVideo key={s.id} slot={s} crop={layout.imageCrops?.[s.panelId]} src={play.src} ratio={play.ratio}/>:null;})}</div>
    </div></div>}
    ...
    {slot&&!imageMode&&<div className="toolbar"><button aria-pressed={!!slot.overflow} disabled={busy} onClick={()=>update(l=>{const s=l.pages[pageIndex].slots.find(s=>s.id===slot.id);if(s.overflow)delete s.overflow;else s.overflow={points:structuredClone(s.points)};},slot.overflow?'枠破りを解除':'枠破り')}>枠破り</button>{slot.overflow&&<label>重ね順<input aria-label="重ね順" type="number" min="0" max="15" disabled={busy} value={slot.overflow.z??0} onChange={e=>update(l=>{const s=l.pages[pageIndex].slots.find(s=>s.id===slot.id);if(!s.overflow)return;s.overflow={...s.overflow,z:Math.max(0,Math.min(15,Math.floor(Number(e.target.value)||0)))};},'はみ出しの重ね順')}/></label>}{motionSources[slot.panelId]?.src&&<button onClick={()=>setFullVideo(motionSources[slot.panelId].src)}>動画の全体を表示</button>}</div>}
