...
export default function LayoutEditor({project,current,commit,run,busy,pageIndex,setPage,model,selected,cancelled,onSelect,active:visible=true}) {
  ...
  useEffect(()=>{setDraft(null);draftRef.current=null;setActive(null);gesture.current=null;setPreviewPageOffset(null);},[candidate?.jobId]);
  useEffect(()=>{
    if(!visible)return;
  ...
  return <section className="layout-editor" aria-label="コマ割り編集">
    ...
    <details ref={proposalDetails}><summary>演出AIでこのページを配置</summary><p>提案が返ると、このキャンバスを仮の配置に切り替えます。採用するまで作品の配置は変わりません。</p><label><input type="checkbox" checked={whole} onChange={e=>setWhole(e.target.checked)}/>全ページを対象（ページ分割・6コマ配置も変更）</label><textarea aria-label="コマ割りの指示" value={instruction} onChange={e=>setInstruction(e.target.value)} placeholder="最初のコマを横長の大ゴマに。最後の境界を斜めに"/><button disabled={busy||!page||!model.connectionId} onClick={()=>run('AIコマ割りを提案中',propose)}>コマ割りを提案</button>{!model.connectionId&&<small>接続・人物設定で演出AIを登録してください。ローカル／クラウド共通です。</small>}</details>
    {candidates.length>0&&<label>保存したコマ割り候補<select aria-label="保存したコマ割り候補" disabled={busy} value={candidate?.jobId??''} onChange={event=>{setCandidateSelection({pageId:page?.id,jobId:event.target.value||null});setPreviewPageOffset(null);setCandidateError('');}}><option value="">採用済みの配置を表示</option>{candidates.map(value=><option key={value.jobId} value={value.jobId}>{value.range?.label??'対象不明'} · {value.reason}{value.invalidReason?'（確認不可）':value.base!==base?'（旧版）':''}</option>)}</select></label>}
    {candidate&&<div className={`layout-candidate ${previewing?'is-preview':''}`} role="status"><strong>{previewing?'AI提案を仮表示中（未採用）':'コマ割り候補を確認'}</strong><p>{candidate.reason}</p>{range&&<p>変更対象: {range.label}。本文・作画・人物は保持。{previewing&&'下のキャンバスは候補の配置です。左のページ一覧は採用済みの配置です。'}</p>}{previewing&&range.count>1&&<div className="toolbar"><button disabled={busy||candidateOffset===0} onClick={()=>setPreviewPageOffset(candidateOffset-1)}>候補の前ページ</button><span>候補 {candidateOffset+1} / {range.count} ページ</span><button disabled={busy||candidateOffset===range.count-1} onClick={()=>setPreviewPageOffset(candidateOffset+1)}>候補の次ページ</button></div>}{previewing&&<label>表示倍率<input aria-label="候補の表示倍率" type="range" min="50" max="160" value={zoom} onChange={e=>setZoom(Number(e.target.value))}/>{zoom}%</label>}{previewing&&!candidateImage&&!candidateError&&<p>候補ページを描画中…</p>}{(candidate.invalidReason||candidateError)&&<p role="alert">候補を表示できません: {candidate.invalidReason??candidateError}</p>}<div className="toolbar"><button disabled={busy||!previewing||!candidateImage||!!candidateError} onClick={()=>run('AI案を採用',async()=>{const at=range.first;await commit(adoptSavedLayoutCandidate(current.current,candidate.jobId));setCandidateSelection({pageId:current.current.layout.pages[at]?.id,jobId:null});setCandidatePreview(null);onSelect?.(null);setPage(at);})}>このコマ割りを採用</button><button disabled={busy} onClick={()=>run('コマ割り候補を破棄',async()=>{await commit(discardLayoutCandidate(current.current,candidate.jobId));setCandidateSelection({pageId:page?.id,jobId:null});setCandidatePreview(null);setCandidateError('');})}>候補を破棄</button></div>{!candidate.invalidReason&&candidate.base!==base&&<p>作品が変更されたため、この候補は採用できません。現在の版から再提案するか、候補を破棄してください。</p>}</div>}
