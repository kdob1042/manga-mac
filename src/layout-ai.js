...
export function validateProposal(project,proposal,scope) {
  ...
  if(!whole&&targets.some(p=>p.locked))throw Error('固定ページはAIコマ割りの変更対象にできません');
  ...
  // Page-scoped proposals may reshape but never change another page or its allocation.
  if(!whole && (proposal.pages.length!==targets.length || proposal.pages.some((p,i)=>p.id!==targets[i].id)))throw Error('指定外のページ変更は採用できません');
  const locked=project.layout.pages.filter(p=>p.locked);
  for(const current of locked) {
    const proposed=proposal.pages.find(p=>p.id===current.id);
    if(whole && (!proposed||JSON.stringify(proposed.slots)!==JSON.stringify(current.slots)))throw Error('AI案が固定ページを変更しています');
  }
  const proposedPages=proposal.pages.map(p=>locked.some(x=>x.id===p.id)?{...p,locked:true}:p);
  const layout={...project.layout,pages:whole?proposedPages:project.layout.pages.map(p=>proposedPages.find(x=>x.id===p.id)??p)};
  validateLayout(layout,project.panels);
  const warnings=layoutWarnings({...layout,pages:proposedPages},project.panels.filter(p=>ids.includes(p.id)));
