...
export function allSourceUnits(snapshot) {
  if (!snapshot?.id || !Array.isArray(snapshot.scenes))
    throw Error('原作スナップショットがありません');
...
export function validateEnglishUnits(value, expected) {
  if (!value || !Array.isArray(value.units))
    throw Error('英訳の形式が不正です');
  if (value.units.length !== expected.length)
    throw Error('英訳に原文の欠落または追加があります');
...
export function currentEnglishLocalization(project, snapshot) {
  return (
    project.localizations?.find(
      (item) =>
        item.locale === ENGLISH_LANGUAGE && item.snapshot_id === snapshot?.id,
    ) ?? null
  );
}

export function createTextResolver(snapshot, localization = null) {
  const source = new Map(
    allSourceUnits(snapshot).map((unit) => [unit.id, unit.text]),
  );
  const translated = localization
    ? new Map(localization.units.map((unit) => [unit.id, unit.text]))
    : null;
  return (unitIds) =>
    unitIds
      .map((id) => {
        ...
        if (
          localization.snapshot_id !== snapshot.id ||
          localization.locale !== ENGLISH_LANGUAGE ||
          !translated.has(id)
        ) {
...
export function textForUnits(unitIds, snapshot, localization = null) {
  return createTextResolver(snapshot, localization)(unitIds);
}
...
export function videoWebVTT(shots, snapshots, localization = null) {
  if (!Array.isArray(shots) || !shots.length)
    throw Error('字幕を書き出す動画ショットがありません');
  const resolvers = new Map();
  const cues = shots.map((shot, index) => {
    if (
      !Number.isInteger(shot.start_ms) ||
      !Number.isInteger(shot.end_ms) ||
