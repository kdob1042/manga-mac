...
import PanelVideo,{VideoLightbox,useMotionSources} from './PanelVideo.jsx';
...
function App() {
  ...
  useEffect(() => { loadProject().then(async p => { if (p) { current.current = p; setProject(p); const s = p.snapshots.find(s => s.id === p.active); if (s?.repo) setRepo(s.repo); } if(desktop()) { const lib=await call('source_library'); setLibrary(lib); const entry=lib.entries.find(e=>e.id===lib.active); if(entry){setRepo(entry.repo);setEpisode(entry.episode);} } setReady(true); }).catch(e => setError(`保存作品を読み込めません: ${e.message}`)); }, []);
  ...
  const motionSources=useMotionSources(project,panels);
  ...
  async function checkSync() {
    setPending(null);
    try {
      if(library) { const entry=library.entries.find(e=>e.id===library.active) ?? {id:library.active,name:current.current.title,repo}; const saved=await call('source_register',{...entry,episode}); setLibrary({...library,entries:saved.entries}); }
  ...
  async function applySync() {
    if(!pending || pending.repo!==repo || pending.episodeId!==episode) throw Error('対象が変わりました。GitHub側の更新を確認してください');
    ...
    await commit({ ...p, title, snapshots: p.snapshots.some(s => s.id === pending.id) ? p.snapshots : [...p.snapshots, pending], active: pending.id, characters });
  ...
  return <div className="app"><header><div className="brand">M<span>↗</span></div><div><strong>Manga Mac</strong><small>原作から、漫画へ。</small></div><div className="spacer"/><span className="local">● {model.provider === 'ollama' ? 'Ollama接続' : '外部LLM接続'}</span><button disabled={!!busy || !ready} onClick={() => setSettings(!settings)}>接続・人物設定</button><button disabled={!!busy || !project.panels.length || !desktop()} onClick={() => run('Live Mangaを書き出し中', async () => { const result = await exportLiveManga(structuredClone(current.current)); setNotice(`Live Manga ${result.releaseId} · ${result.path} · 検証済み`); })}>Live Mangaを書き出す</button><button disabled={!!busy || !project.panels.length} onClick={() => run('書き出し中', async () => download(await exportCBZ(project), `manga-${project.output_locale}.cbz`))}>CBZを書き出す ↗</button></header>
    ...
    <div hidden={layoutMode}>{!panels.length ? <div className="welcome"><div className="welcome-icon">▤</div><h1>物語の、その先を描こう。</h1><p>完成した脚本と、キャラクターの参照画像。<br/>ふたつをつないで、最初のページをつくります。</p><button className="primary" onClick={() => setSettings(true)}>原作を接続する →</button>{!desktop() && <button disabled={!!busy || !ready} onClick={() => run('サンプルを開く', sample)}>画面のサンプルを見る</button>}<small>脚本はGitHubのmainと読み取り専用で同期します。</small></div> : <div className="page"><div className="panel-grid">{panels.map((p, i) => <article key={p.id} className={`panel ${selected === p.id ? 'selected' : ''}`} onClick={() => { if (selected !== p.id) { setSelected(p.id); setRect(null); } }}><div className="art" onPointerDown={e => { if (busy || !p.image) return; setSelected(p.id); drag.current = point(e); e.currentTarget.setPointerCapture(e.pointerId); setRect(null); }} onPointerUp={e => { if (!drag.current) return; const end = point(e), start = drag.current; drag.current = null; const r = [Math.min(start[0], end[0]), Math.min(start[1], end[1]), Math.abs(end[0] - start[0]), Math.abs(end[1] - start[1])]; if (r[2] > .01 && r[3] > .01) setRect(r); }}>{p.image ? (motionSources[p.id]?.src && pageData?.slots.find(s=>s.panelId===p.id) ? <PanelVideo mode="tile" slot={pageData.slots.find(s=>s.panelId===p.id)} crop={project.layout.imageCrops?.[p.id]} src={motionSources[p.id].src} ratio={motionSources[p.id].ratio}/> : <img draggable="false" src={p.image} alt={`コマ ${i + 1}`}/>) : <div className="placeholder"><span>0{i + 1}</span><p>作画を待っています</p></div>}{selected === p.id && rect && <div className="region" style={{ left: `${rect[0] * 100}%`, top: `${rect[1] * 100}%`, width: `${rect[2] * 100}%`, height: `${rect[3] * 100}%` }}/>}</div><div className={`caption ${project.output_locale === 'en' && !project.localizations.some(item => item.locale === 'en' && item.snapshot_id === p.snapshotId) ? 'missing-translation' : ''}`}>{panelText(p)}</div><div className="panel-meta">{p.sceneId} · {p.status === 'review' ? '見た目の確認待ち' : '演出計画'}</div></article>)}</div><div className="folio">{page + 1}</div></div>}
    </div>
    {chosen && !layoutMode && <PanelMotionControls project={project} panel={chosen} current={current} commit={commit} run={run} busy={!!busy} onShot={id => { setRequestedShot(id); setMedium('video'); }} playback={motionSources[chosen.id]} onExpand={()=>setFullVideo(motionSources[chosen.id]?.src)}/>}
    {fullVideo&&<VideoLightbox src={fullVideo} onClose={()=>setFullVideo(null)}/>}
