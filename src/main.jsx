...
function App() {
  ...
  const [repo, setRepo] = useState('kdob1042/Kamiya-Kawai'), [token, setToken] = useState(''), [episode, setEpisode] = useState('P01'), [model, setModel] = useState('qwen3:8b');
  ...
  async function edit() {
    ...
    if (!rect) throw Error('画像をドラッグして、変更してよい範囲を指定してください');
    const next = await editRegion(chosen, project.characters, instruction, rect);
  ...
  return <div className="app"><header><div className="brand">M<span>↗</span></div><div><strong>Manga Mac</strong><small>原作から、漫画へ。</small></div><div className="spacer"/><span className="local">● ローカル制作</span><button disabled={!!busy || !ready} onClick={() => setSettings(!settings)}>接続・人物設定</button><button disabled={!!busy || !project.panels.length} onClick={() => run('書き出し中', async () => download(await exportCBZ(project), 'manga.cbz'))}>CBZを書き出す ↗</button></header>
    ...
    <main><div className="toolbar"><span>{project.panels.length ? `PAGE ${String(page + 1).padStart(2, '0')}` : '制作をはじめましょう'}</span><div className="spacer"/><button disabled={!!busy || !project.history.length} onClick={() => run('元に戻す', async () => { const p = current.current, h = p.history.at(-1); await commit({ ...p, panels: h.panels, history: p.history.slice(0, -1) }); })}>↶ 元に戻す</button><button disabled={!!busy || !panels.length} onClick={() => run('PNGを書き出し', async () => { const data = await pagePNG(panels, project.snapshots); download(new Blob([Uint8Array.from(atob(data.split(',')[1]), c => c.charCodeAt(0))], { type: 'image/png' }), `page-${page + 1}.png`); })}>PNG</button></div>
    {error && <div role="alert" className="message error">{error}</div>}{notice && <div role="status" className="message">{notice}</div>}{busy && <div role="status" className="message progress">◌ {busy}<button onClick={() => { cancel.current = true; setNotice('現在の処理が終わったところで停止します'); }}>ここまでで停止</button></div>}
    ...
    <div className="composer"><div className="scope">{chosen ? `選択中：${chosen.sceneId} ／ ${rect ? '指定領域だけを変更' : '画像をドラッグして範囲を指定'}` : 'コマを選択して、気になるところを直す'}</div><div className="input-row"><input value={instruction} onChange={e => setInstruction(e.target.value)} placeholder="もう少し照れた笑顔に。髪型と制服はそのままで。"/><button className="primary" disabled={!!busy || !chosen?.image || !rect || !instruction.trim()} onClick={() => run('選択範囲を修正中', edit)}>修正する ↑</button></div><small>指定範囲の外側は元画像を保持します。台詞本文はGitHub側で変更してください。</small></div></main>
    {settings && <section className="settings"><div className="setting-head"><h2>制作の準備</h2><button onClick={() => setSettings(false)}>閉じる</button></div><h3>01 / 原作をつなぐ</h3><label>GitHubリポジトリ<input value={repo} onChange={e => setRepo(e.target.value)}/></label><label>話ID<input value={episode} onChange={e => setEpisode(e.target.value)} placeholder="P01"/></label><label>読み取り専用トークン<input type="password" autoComplete="off" value={token} onChange={e => setToken(e.target.value)}/></label><small>トークンは今回の起動中だけ保持。原作リポジトリのContents: readを使用します。</small><button className="full" disabled={!!busy || !ready} onClick={() => run('原作を取得中', checkSync)}>更新を確認する</button><h3>02 / キャラクターの正本</h3><div className="characters">{project.characters.map(c => <div key={c.id}><img src={c.image}/><span>{c.name}<small>v{c.version} · {c.hash.slice(0, 8)}</small></span></div>)}</div><label>人物名<input value={name} onChange={e => setName(e.target.value)} placeholder="河合由美"/></label><label>固定する特徴<textarea value={description} onChange={e => setDescription(e.target.value)} placeholder="髪型・体格・衣装など"/></label><label className="file">＋ 正本画像を登録<input type="file" accept="image/png,image/jpeg,image/webp" disabled={!!busy || !ready} onChange={e => { const file = e.target.files[0]; run('参照を登録', () => addCharacter(file)); e.target.value = ''; }}/></label><h3>03 / ローカルAI</h3><label>Ollamaの導入済みモデル<input value={model} onChange={e => setModel(e.target.value)}/></label><button disabled={!!busy} className="full" onClick={() => run('画像モデルを準備中（初回ダウンロード）', async () => { await call('prepare_engine'); setNotice('画像モデルの準備が完了しました'); })}>画像モデルを準備する</button><small>FLUX.2 klein 4B。初回はネット接続と十分な空き容量が必要です。Ollamaは別途起動してください。</small><h3>作品のバックアップ</h3><button disabled={!ready || !!busy} onClick={() => download(new Blob([JSON.stringify(project)], { type: 'application/json' }), 'manga-project.json')}>作品データを書き出す</button></section>}
    </div></div>;
}
