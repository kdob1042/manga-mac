...
function App() {
  ...
  async function edit() {
    ...
    let region = rect;
    if (!region) { const character = project.characters.find(c => c.id === target && chosen.characterIds.includes(c.id)); if (!character) throw Error('顔を直す人物を選ぶか、画像をドラッグして範囲を指定してください'); setBusy('正本と照合して顔の範囲を特定中'); region = await locateFace(chosen, character, visionModel); setRect(region); setNotice('完成画像上の修正範囲を確認し、もう一度「修正」を押してください'); return; }
  ...
  return <div className="app"><header><div className="brand">M<span>↗</span></div><div><strong>Manga Mac</strong><small>原作から、漫画へ。</small></div><div className="spacer"/><span className="local">● {model.provider === 'ollama' && visionModel.provider === 'ollama' ? 'Ollama接続' : '外部LLM接続'}</span><button disabled={!!busy || !ready} onClick={() => setSettings(!settings)}>接続・人物設定</button><button disabled={!!busy || !project.panels.length} onClick={() => run('書き出し中', async () => download(await exportCBZ(project), `manga-${project.output_locale}.cbz`))}>CBZを書き出す ↗</button></header>
    ...
    {chosen && <LetteringControls key={`${chosen.id}:${project.revision}`} panel={chosen} current={current} commit={commit} run={run} busy={!!busy}/>}
