...
function App() {
  ...
  async function checkSync() { if(library) {const entry=library.entries.find(e=>e.id===library.active);const saved=await call('source_register',{...entry,episode});setLibrary({...library,entries:saved.entries});} const s = await syncSource(repo, token, episode, snapshot); if (s.id === snapshot?.id) setNotice('原作は最新です'); else setPending(s); }
