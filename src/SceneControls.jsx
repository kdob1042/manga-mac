...
export default function SceneControls({project,panel,current,commit,run,busy,model,active=true}){
  const [opened,setOpened]=useState(false),[selected,setSelected]=useState(''),[editMode,setEditMode]=useState('translate'),[newAsset,setNewAsset]=useState(''),[assetKind,setAssetKind]=useState('prop'),[instruction,setInstruction]=useState(''),[rigInfo,setRigInfo]=useState({}),[captureWidth,setCaptureWidth]=useState(()=>suggestedSize(project,panel.id)[0]),[captureHeight,setCaptureHeight]=useState(()=>suggestedSize(project,panel.id)[1]);
  ...
  return <section className="scene-controls" aria-label="3D構図編集">
    ...
    <Suspense fallback={<p role="status">3D画面を読み込み中…</p>}><SceneViewport ref={viewport} scene={scene} assets={assets} onCamera={saveCamera} onRigInfo={setRigInfo} selectedObjectId={chosen?.id} editMode={editMode} onTransform={(id,values)=>update({type:'transform',id,...values})}/></Suspense>
    ...
    <Suspense fallback={null}><ScenePoseControls scene={scene} assets={assets} chosen={chosen} rigInfo={rigInfo[chosen?.id]} update={update} busy={busy}/></Suspense>
