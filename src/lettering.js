...
export function validateLettering(panel, layout) {
  ...
  for (const box of layout.boxes) {
    ...
    if (box.id !== undefined && box.id !== `letter:${box.unit_id}`) throw Error('文字枠IDは原文参照から変更できません');
    if (Object.keys(box).some(k => !['id','unit_id','x','y','width','height','shape','tail','fontSize','lineHeight','padding','locked'].includes(k))) throw Error('文字枠の未対応項目です');
