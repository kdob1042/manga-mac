...
export async function call(command, args = {}, permit=null) {
  ...
  if((command==='generate_image'||command==='prepare_engine'||command==='prepare_media_engine')&&!holdsResource(permit,'local-inference'))return withResource('local-inference',1,()=>invoke(command,args));
