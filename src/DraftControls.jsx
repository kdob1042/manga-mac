...
export default function DraftControls({project,current,commit,run,busy,onSwitch,onProduce,model,cancelled,canProduce=true,sourceToken='',episodeId=''}) {
  ...
  return <div className="draft-controls">
    ...
    <button className={project.namePlan?'primary full':'full'} disabled={busy||!canProduce||!ids.length||project.namePlan?.status==='stale'} onClick={()=>run('制作を開始',async()=>{if(current.current.namePlan?.format!=='manga-mac/name-plan/v2'||separate)await commit(startDraft(current.current,ids,separate));setSeparate(false);onSwitch();await onProduce();})}>{project.namePlan?.format==='manga-mac/name-plan/v2'&&!separate?'このネームで制作':'✧ 漫画にする'}</button>
  </div>;
}
