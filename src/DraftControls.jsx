...
export default function DraftControls({project,current,commit,run,busy,onSwitch,onProduce,model,cancelled,canProduce=true,sourceToken='',episodeId=''}) {
  ...
  return <div className="draft-controls">
    <details><summary>制作する場面・保存した原稿</summary>
      ...
      <NamePlanControls project={project} current={current} commit={commit} run={run} busy={busy} model={model} sceneIds={ids} onSwitch={()=>{setSelection(null);setSeparate(false);onSwitch();}} cancelled={cancelled} sourceToken={sourceToken} episodeId={episodeId}/>
    </details>
