...
export default function PanelMotionControls({project,panel,current,commit,run,busy,onShot,playback,onExpand}) {
 ...
 <button disabled={!project.motionHistory?.some(h=>h.panelId===panel.id)} onClick={()=>run('割当を元に戻す',()=>commit(undoMotion(current.current,panel.id)))}>割当を元に戻す</button>
 {playback?.src&&<button onClick={onExpand}>動画の全体を表示</button></fieldset></section>;
}
