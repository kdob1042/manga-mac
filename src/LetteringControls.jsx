...
import { drawLettering, drawNameLettering, pagePNG, pageLayers } from './render.js';
...
export default function LetteringControls({ panel, current, commit, run, busy, model, pageIndex=0, active=true }) {
  ...
  const nameFrame=panel.namePlanVersion===2?letteringFrame(current.current,panel.id):null;
  const viewWidth=nameFrame?.width??720,viewHeight=nameFrame?.height??720;
  const defaultFontSize=panel.namePlanVersion===2?48:24;
  ...
  function addNarration() {
    ...
    const next={id,text:'ナレーション',kind:'narration',shape:'rect',tail:null,x:.28,y:.24,width:.44,height:.16,...(panel.namePlanVersion===2?{fontSize:48}:{})};
  ...
  useEffect(()=>{
    ...
    async function paint() {
      ...
      const surface=document.createElement('canvas');surface.width=Math.ceil(viewWidth);surface.height=Math.ceil(viewHeight);
      const ctx=surface.getContext('2d');ctx.scale(surface.width/viewWidth,surface.height/viewHeight);
      ctx.fillStyle='#f5f3ef';ctx.fillRect(0,0,viewWidth,viewHeight);
      ...
      if(panel.image){
        ...
        if(stale)return;
        if(nameFrame)ctx.drawImage(im,frame.x,frame.y,frame.width,frame.height,0,0,viewWidth,viewHeight);
        else ctx.drawImage(im,frame.x,frame.y,frame.width,frame.height,2,2,716,716);
      }
      if(nameFrame){
        const slot=p.layout.pages[pageIndex].slots.find(s=>s.panelId===panel.id);
        ctx.save();ctx.translate(-nameFrame.x,-nameFrame.y);
        await drawNameLettering(ctx,{...panel,lettering:layout},slot,p.snapshots,p.localizations,p.output_locale);
        ctx.restore();
      }else{
        ...
        for(const b of layout.boxes){
          ...
          await drawLettering(ctx,value,{x:2+b.x*716,y:2+b.y*716,width:b.width*716,height:b.height*716},true,b);
        }
      }
      if(stale||!canvas.current)return;
      const visible=canvas.current;visible.width=surface.width;visible.height=surface.height;
      visible.getContext('2d').drawImage(surface,0,0);
      setProblem('');
    }
    if(layout.mode==='balloons')paint().catch(e=>{if(!stale)setProblem(e.message);});
    return ()=>{stale=true;};
  },[active,layout,panel.image,current.current.layout,current.current.output_locale,current.current.localizations,viewWidth,viewHeight]);
  ...
  return <fieldset className="shot-controls" disabled={busy}><legend>文字配置</legend>
    {panel.namePlanVersion!==2&&<label>配置方法<select value={layout.mode} onChange={e=>setLayout({...layout,mode:e.target.value})}><option value="caption">絵の下に本文</option><option value="balloons">コマ内（吹き出し・ナレーション）</option></select></label>}
    {layout.mode==='balloons'&&<>
      <button type="button" onClick={addNarration}>ナレーション枠を追加</button>
      {box&&<div className="lettering-stage" style={{aspectRatio:`${viewWidth} / ${viewHeight}`}}><canvas ref={canvas} width={Math.ceil(viewWidth)} height={Math.ceil(viewHeight)}/>
        <svg viewBox="0 0 1 1" preserveAspectRatio="none" tabIndex="0" aria-label="吹き出し直接編集" onKeyDown={e=>{if(e.key==='Escape')reset();}}
      ...
      {box&&<>
        ...
        <label>文字方向<select aria-label="文字方向" disabled={box.locked} value={box.writingMode??'horizontal-tb'} onChange={e=>change({writingMode:e.target.value})}><option value="horizontal-tb">横書き</option><option value="vertical-rl">縦書き</option></select></label>
        <label>書体<select aria-label="書体" disabled={box.locked} value={box.fontFamily??'gothic'} onChange={e=>change({fontFamily:e.target.value})}><option value="gothic">ゴシック</option><option value="mincho">明朝</option></select></label>
        {box.writingMode==='vertical-rl'&&<small>右から左へ列を送ります。英数字は1文字ずつ縦に配置します。ルビ・縦中横は未対応です。</small>}
        {kind==='narration'&&<small>ナレーション枠はコマ内の任意位置へ配置できます。下部固定ではありません。</small>}
        {[['x','横位置',0,1,.01],['y','縦位置',0,1,.01],['width','幅',.08,1,.01],['height','高さ',.06,1,.01],['fontSize','文字サイズ',panel.namePlanVersion===2?32:14,72,1],['lineHeight','行間',1,2,.05],['padding','余白',0,40,1]].map(([key,label,min,max,step])=><label key={key}>{label}<input aria-label={label} disabled={box.locked} type="number" min={min} max={max} step={step} value={box[key]??({fontSize:defaultFontSize,lineHeight:1.25,padding:12}[key])} onChange={e=>change({[key]:Number(e.target.value)})}/></label>)}
