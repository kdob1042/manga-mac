...
import { defaultLettering, letteringKind, setLettering, validateLettering } from './lettering';
  ...
  return <fieldset className="shot-controls" disabled={busy}><legend>文字配置</legend>
    ...
    {layout.mode==='balloons'&&box&&<>
      <div className="lettering-stage"><canvas ref={canvas} width="720" height="720"/>
        <svg viewBox="0 0 1 1" tabIndex="0" aria-label="文字枠直接編集" onKeyDown={e=>{if(e.key==='Escape')reset();}}
      ...
      <label>文字種別<select aria-label="文字種別" disabled={box.locked} value={letteringKind(box)} onChange={e=>{const kind=e.target.value;change({kind,...(kind==='narration'?{shape:'rect'}:{}),...(kind!=='balloon'?{tail:null}:{})});}}><option value="balloon">吹き出し</option><option value="thought">心中描写（枠なし）</option><option value="narration">ナレーション（四角）</option></select></label>
      ...
      <label>形状<select disabled={box.locked||letteringKind(box)!=='balloon'} value={letteringKind(box)==='narration'?'rect':(box.shape??'round')} onChange={e=>change({shape:e.target.value})}><option value="round">角丸</option><option value="rect">長方形</option><option value="ellipse">楕円</option></select></label>
      <label><input type="checkbox" checked={!!box.locked} onChange={e=>change({locked:e.target.checked})}/>この文字枠を固定</label>
      {letteringKind(box)==='balloon'&&<button disabled={box.locked} onClick={()=>change({tail:box.tail?null:[Math.min(1,box.x+box.width/2),Math.min(1,box.y+box.height+.08)]})}>{box.tail?'しっぽを外す':'しっぽを付ける'}</button>}
    </>}
    ...
    <small>ドラッグは1操作で保存。Escapeで取消。本文と順序を保持します。文字種別は枠ごとに選べ、心中描写は枠なし、ナレーションは四角枠です。画像送信を有効にすると認識した顔・手の領域を避けます。認識結果の見た目も確認してください。</small>
  </fieldset>;
}
