    ...
    {layout.mode==='balloons'&&box&&<>
      <div className="lettering-stage"><canvas ref={canvas} width="720" height="720"/>
        <svg viewBox="0 0 1 1" tabIndex="0" aria-label="吹き出し直接編集" onKeyDown={e=>{if(e.key==='Escape')reset();}}
          ...
          onPointerCancel={reset} onLostPointerCapture={reset} onPointerUp={()=>{const d=drag.current;if(!d)return;drag.current=null;run('文字配置を保存中',async()=>{if(d.base!==editBase(current.current))throw Error('編集中に作品が変わりました');await save(layout);});}}>
