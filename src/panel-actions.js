...
export async function checkPanelAction(project,op,imageModelId = null) {
  ...
  if(['finishing','upscale','resolution'].includes(op.kind)) {
    ...
    if(op.kind==='finishing')await beginFinishing(project,op.panelId,im.width,im.height,imageModelId);
    else if(op.kind==='upscale')await beginUpscale(project,op.panelId,im.width,im.height,op.args.factor);
    else finishingPlan(project,op.panelId,im.width,im.height,imageModelId);
  }
...
export async function prepareFinishing(current,commit,id,imageModelId = null) {
  const p=current(),source=p.panels.find(x=>x.id===id),im=await imageOf(source.image),job=await beginFinishing(p,id,im.width,im.height,imageModelId);
  await commit({...p,jobs:[...p.jobs,job]});
  try {const result=await generatePanel(source,p.characters,source.image,finishingInstruction(job),job,null,p.style_references??[],null,null,imageModelId);await commit(await finishJob(current(),job,result,false,true));}
...
export async function panelAction(current,commit,op,onShot,imageModelId = null) {
  if(op.kind==='finishing'){await prepareFinishing(current,commit,op.panelId,imageModelId);return '仕上げ候補を保存しました。対象コマで比較して採用してください';}
  ...
  if(op.kind==='resolution') {
    const p=current(),source=p.panels.find(p=>p.id===op.panelId),im=await imageOf(source.image),plan=finishingPlan(p,op.panelId,im.width,im.height,imageModelId);
