...
export function finishingPlan(project,id,width,height) {
  ...
  const sizes=[];
  for(let w=256;w<=1024;w+=64)for(let h=256;h<=1024;h+=64)if(w*height===h*width)sizes.push({width:w,height:h});
  ...
  return {method:'reference-regeneration',sourceWidth:width,sourceHeight:height,requiredWidth,requiredHeight,...output,
    sufficient:output.width>=requiredWidth&&output.height>=requiredHeight,sourceSufficient:scale<=1,scale};
}
export async function beginFinishing(project,id,width,height) {
  const panel=project.panels.find(p=>p.id===id);
  if(!panel?.image)throw Error('作画済みのコマを選択してください');
  const plan=finishingPlan(project,id,width,height);
  return {...await beginJob(project,panel,'retake'),placement_key:placementKey(project,id),
