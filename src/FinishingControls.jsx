...
export default function FinishingControls({project,panel,current,commit,run,busy,imageModelId,active=true}) {
  ...
  return <section className="shot-controls" aria-label="配置に合わせた仕上げ"><h3>配置に合わせた仕上げ</h3>
    ...
    {error&&<p>{error}</p>}
    {plan&&<><p>元画像 {plan.sourceWidth} × {plan.sourceHeight} px ／ 標準幅1600pxに必要な原画像サイズ {plan.requiredWidth} × {plan.requiredHeight} px</p>
      <p>{plan.sourceSufficient?'標準幅1600pxでは解像度が足りています。':'標準幅1600pxでは解像度が不足しています。'} 選んだ出力幅の不足は「書き出す」から確認できます。</p>
