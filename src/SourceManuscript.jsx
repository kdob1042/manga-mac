...
export default function SourceManuscript({project,busy=false,onApply,current}){
 ...
 async function apply(){try{setError('');const latest=current?.()??project;await onApply(sourceSelection(latest,changes,ids));}catch(e){setError(e.message);}}
 return <section className="source-manuscript" aria-label="原稿"><h2>原稿</h2><p>原稿 {snapshot.sha?.slice(0,8)??snapshot.id} · 読み取り専用。漫画への反映状態を表示しています。</p>
  <div className="toolbar"><button disabled={busy||!changes.blocks.length} onClick={()=>setSelection({id:changes.id,ids:changes.blocks.map(b=>b.id)})}>未反映・削除対象をすべて選択</button><button disabled={busy||!ids.length} onClick={()=>setSelection({id:changes.id,ids:[]})}>全解除</button><span role="status">{ids.length} / {changes.blocks.length} ブロックを選択</span><button disabled={busy||!ids.length||!onApply} onClick={apply}>選択箇所を漫画に反映</button></div>
  {!onApply&&<p>差分の確認・選択ができます。漫画への反映は準備中です。</p>}{error&&<p role="alert">{error}</p>}
  {!rows.length&&<p>本文はありません。</p>}
  {rows.map((row,i)=>row.type==='applied'?<article className="source-applied" key={`applied:${i}`}><strong>反映済み · {row.ref.sceneId}</strong><pre className="source-text">{resolve(row.ref)}</pre></article>:row.type==='move-origin'?<aside key={`origin:${row.block.id}`} className="source-deletion"><strong>削除対象 · 移動元</strong><p>移動先と同時に反映します。</p><pre className="source-text">{oldText(row.oldRefs)}</pre><button disabled={busy} aria-pressed={ids.includes(row.block.id)} onClick={()=>toggle(row.block.id)}>移動元・移動先をまとめて{ids.includes(row.block.id)?'解除':'選択'}</button></aside>:<article key={row.block.id} className="source-change">
   <label className="source-select"><input type="checkbox" disabled={busy} checked={ids.includes(row.block.id)} onChange={()=>toggle(row.block.id)}/>{row.block.kind==='delete'?'削除対象':row.block.kind==='replace'?'未反映 · 変更':row.block.kind==='move'?'未反映 · 移動':'未反映 · 追加'} · {[...new Set([...row.oldRefs,...row.block.newRefs].map(r=>r.sceneId))].join(' / ')}</label>
