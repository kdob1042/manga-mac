...
export default function SourceManuscript({project,busy=false,onApply,current,pendingBlockIds=[]}){
 ...
 return <section className="source-manuscript" aria-label="原稿"><h2>原稿</h2><p>青：未着手・未反映 / 通常：制作中 / 緑：完了 / 黄：要確認 / 赤：削除対象</p><p>原稿 {snapshot.sha?.slice(0,8)??snapshot.id} · 読み取り専用。漫画への反映状態を表示しています。</p>
  ...
  {rows.map((row,i)=>row.type==='applied'?<article className={`source-applied ${sectionStatus(project,row.ref.sceneId)==='完了'?'section-complete':sectionStatus(project,row.ref.sceneId)==='要確認'?'section-review':''}`} key={`applied:${i}`}><strong>{sectionStatus(project,row.ref.sceneId)} · 反映済み · {row.ref.sceneId}</strong><pre className="source-text">{resolve(row.ref)}</pre></article>:row.type==='move-origin'?<aside key={`origin:${row.block.id}`} className="source-deletion"><strong>削除対象 · 移動元</strong><p>移動先と同時に反映します。</p><pre className="source-text">{oldText(row.oldRefs)}</pre><button disabled={busy||pendingBlockIds.includes(row.block.id)} aria-pressed={ids.includes(row.block.id)} onClick={()=>toggle(row.block.id)}>移動元・移動先をまとめて{ids.includes(row.block.id)?'解除':'選択'}</button></aside>:<article key={row.block.id} className="source-change">
