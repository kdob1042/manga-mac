...
import {tagOptions,matchingScenes,rowSceneIds,tagBlockSelection} from './source-tags.js';
...
export default function SourceManuscript({project,busy=false,onApply,current,pendingBlockIds=[]}){
 ...
 return <section className="source-manuscript" aria-label="原稿"><h2>原稿</h2><p>原稿 {snapshot.sha?.slice(0,8)??snapshot.id} · 読み取り専用。漫画への反映状態を表示しています。</p>
  ...
  {expansion&&<div role="alert">関連差分のため追加対象が必要です: {expansion.additional.join(' / ')}<button onClick={()=>{setSelection({id:changes.id,ids:expansion.ids});setExpansion(null);}}>追加対象を含めて選択</button><button onClick={()=>setExpansion(null)}>取消</button></div>}
