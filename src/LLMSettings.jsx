...
export default function LLMSettings({ title, value, onChange, vision, disabled, run, notify, t }) {
  ...
  return <fieldset disabled={disabled} className="llm-settings"><legend>{title}</legend>
    <label>{t('connectionTarget')}<select aria-label={`${title} ${t('connectionTarget')}`} value={value.provider} onChange={e => replace(defaultConnection(e.target.value, vision))}>{Object.keys(providers).map(id => <option key={id} value={id}>{providerLabel(id)}</option>)}</select></label>
    ...
    <label>{t('modelId')}<input aria-label={`${title} ${t('modelId')}`} value={value.model} onChange={e => change({ model: e.target.value })} placeholder={vision ? t('visionModelPlaceholder') : t('modelPlaceholder')}/></label>
    {external && <><label>{t('apiKey')}<input type="password" autoComplete="off" aria-label={`${title} ${t('apiKey')}`} value={value.apiKey} onChange={e => change({ apiKey: e.target.value })}/></label><small>{t('destination', { destination: value.provider === 'custom' ? value.baseUrl || t('unset') : providers[value.provider].baseUrl })}<br/>{vision ? t('visionDisclosure') : t('planningDisclosure')} {t('apiCost')}</small>{value.provider === 'custom' && <label><input type="checkbox" checked={value.jsonMode} onChange={e => change({ jsonMode: e.target.checked })}/> {t('jsonMode')}</label>}</>}
