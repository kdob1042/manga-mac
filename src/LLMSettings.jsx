...
export default function LLMSettings({ title, value, onChange, vision, disabled, run, notify }) {
  ...
  return <fieldset disabled={disabled} className="llm-settings"><legend>{title}</legend>
    <label>接続先<select aria-label={`${title}の接続先`} value={value.provider} onChange={e => replace(defaultConnection(e.target.value, vision))}>{Object.entries(providers).map(([id, p]) => <option key={id} value={id}>{p.label}</option>)}</select></label>
    ...
    <button type="button" onClick={() => run('LLMの接続を確認中', async () => { const registered = value.connectionId ? value : await registerConnection(value); onChange(registered); const result = JSON.parse(await askLLM(registered, { purpose: 'probe', prompt: 'Return {"ok":true}.', schema: { type: 'object', properties: { ok: { type: 'boolean' } }, required: ['ok'] } })); if (result.ok !== true) throw Error('接続先のJSON応答が不正です'); notify('LLMへの接続とJSON応答を確認しました。画像対応の確認は実際の顔選択時に行います。'); })}>接続をテスト</button>
  </fieldset>;
}
