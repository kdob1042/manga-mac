...
import {defaultCrop,containCrop,coverCrop,cropRect,panCrop,validateCrop} from '../src/image-crop.js';
...
test('missing crop covers the frame; contain remains an explicit opt-out',()=>{
 assert.deepEqual(coverCrop(undefined),defaultCrop());
 assert.equal(coverCrop(containCrop()),null);
 const p=ensureLayout({panels:[{id:'p',image:'original',lettering:{mode:'balloons'},unitIds:['u']}],jobs:[]});
 const l={...p.layout,imageCrops:{p:containCrop()}};
 assert.equal(validateLayout(l,p.panels),l);
});
test('crop history, reload and removal do not touch original artwork, lettering, source or jobs',()=>{
 ...
 for(const crop of [null,{}, {zoom:NaN,x:0,y:0},{zoom:9,x:0,y:0},{zoom:1,x:-1,y:0},{zoom:1,x:0,y:Infinity},{zoom:1,x:.5,y:.5,fit:'cover'}])assert.throws(()=>validateLayout({...l,imageCrops:{p:crop}},p.panels));
