...
test('prepare motion from the selected panel uses only its source and switches to the saved shot',async({page})=>{
 await page.goto('/');await page.evaluate(async legacy=>{const {saveProject}=await import('/src/bridge.js');await saveProject(legacy);},legacy);await page.reload();await page.locator('.panel').first().click();const section=page.getByRole('region',{name:'コマの動画'});await section.getByLabel('動きの指示').fill('小さくうなずく');await section.getByRole('button',{name:'この作画から動画を準備'}).click();await expect(page.getByRole('button',{name:'動画',exact:true})).toHaveAttribute('aria-pressed','true');await expect(page.getByRole('heading',{name:/5秒/})).toBeVisible();
