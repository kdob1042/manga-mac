...
test('one action directs four isolated shots, draws them, and edits without a face model', async ({ page }) => {
  ...
  await page.getByPlaceholder('もう少し照れた笑顔に。髪型と制服はそのままで。').fill('口元を修正');
  await expect(page.getByRole('button',{name:'修正する ↑',exact:true})).toBeDisabled();
  ...
  expect(requests.every(p=>['probe','direction'].includes(p))).toBe(true);
});
