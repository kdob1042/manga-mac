...
test('art page uses the saved export geometry and selects panels without changing the proof',async({page})=>{
  ...
  await page.getByRole('button',{name:'このコマの枠サイズを調整'}).click();
  await expect(page.getByRole('button',{name:'コマ割り編集',exact:true})).toHaveAttribute('aria-pressed','true');
  const slot=page.getByTestId('layout-slot-5');
  ...
  const handle=await page.getByTestId('edge-1').boundingBox();
