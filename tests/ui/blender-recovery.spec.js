...
for (const action of ['adopt', 'abandon']) {
  test(`Blender unknown job can be ${action} without a new execution`, async ({ page }) => {
    ...
    await expect(page.getByText('旧保存処理の確認', {exact:false})).toBeVisible();
    await page.getByRole('button', { name: action === 'adopt' ? '保存結果を検証して採用' : '採用せずに解消', exact: true }).click();
    await expect(page.getByText('旧保存処理の確認', {exact:false})).toHaveCount(0);
    await expect(page.getByRole('button', {name:'撮影する',exact:true})).toHaveCount(0);
