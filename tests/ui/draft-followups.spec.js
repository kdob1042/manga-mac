...
test('selected-scene draft retains old manuscript; saved edit comparison survives restart without connection',async({page})=>{
 ...
 await page.getByText('制作する場面・ネーム・保存した原稿',{exact:true}).click();await page.getByRole('group',{name:'制作する場面',exact:true}).getByLabel('a',{exact:true}).uncheck();await page.getByLabel('既存原稿を残して別の初稿を作る').check();
 ...
 await page.getByRole('button',{name:'原稿',exact:true}).click();await page.getByText('制作する場面・ネーム・保存した原稿',{exact:true}).click();const id=await page.evaluate(()=>window.saved.history.find(h=>h.draftCheckpoint).id);await page.getByLabel('保存した原稿',{exact:true}).selectOption(id);
