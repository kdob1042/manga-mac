...
test('overflow art covers the neighbor home while live clip stays on the overflowing panel frame',async({page})=>{
 await page.goto('/');
 const sample=await page.evaluate(async fixture=>{
  const {saveProject,loadProject}=await import('/src/bridge.js');
  const color=(c)=>{const canvas=document.createElement('canvas');canvas.width=768;canvas.height=768;const ctx=canvas.getContext('2d');ctx.fillStyle=c;ctx.fillRect(0,0,768,768);return canvas.toDataURL('image/png');};
  fixture.panels=[{...fixture.panels[0],id:'red',unitIds:[],sourceRefs:[],image:color('#ff0000')},{...fixture.panels[0],id:'blue',unitIds:[],sourceRefs:[],image:color('#0000ff')}];
  fixture.history=[];fixture.jobs=[];
  await saveProject(fixture);
  ...
  return {covered,uncovered,changed:data!==baseline,warnings:layoutWarnings(p.layout,p.panels),clip:preview.preview.manifest.pages[0].panels[0].clip,home};
 },legacy);
 expect(sample.warnings).toEqual([]);
 expect(sample.changed).toBe(true);
 expect(sample.covered[0]).toBeGreaterThan(200);
 expect(sample.covered[2]).toBeLessThan(80);
 expect(sample.uncovered[2]).toBeGreaterThan(200);
 expect(sample.uncovered[0]).toBeLessThan(80);
