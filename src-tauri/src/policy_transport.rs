...
fn rejected() -> http_client::Error {
    http_client::Error::Instance(Box::new(std::io::Error::other(
        "LLM transport rejected the request",
    )))
}
...
impl PolicyTransport {
    ...
    pub async fn external_client(url: &reqwest::Url) -> Result<reqwest::Client, String> {
        ...
        Self::new(&url.origin().ascii_serialization(), false, "")
            .await?
            .client
            .ok_or("Missing HTTP client".into())
    }
    pub async fn new(endpoint: &str, local: bool, path: &str) -> Result<Self, String> {
        ...
        let port = url
            ...
            .ok_or("Endpoint port is required")?;
        let addresses: Vec<SocketAddr> = if local {
            ...
            if url.scheme() != "https" || port != 443 {
                return Err("External endpoints require HTTPS on port 443".into());
            }
            ...
            .map_err(|_| "Endpoint resolution failed")?
            .collect();
        ...
        Ok(Self {
            target: format!("{}{path}", endpoint.trim_end_matches('/')),
            client: Some(client),
        ...
        U: From<Bytes> + Send + 'static,
    {
        ...
        async move {
            ...
            if url.as_str() != target
                ...
                || url.query().is_some()
                || url.fragment().is_some()
                || parts.headers.contains_key("host")
                || parts.headers.contains_key("proxy-authorization")
                || parts.method != http_client::Method::POST
...
mod tests {
    ...
    async fn policy_rejects_unapproved_origins_before_sending() {
        for endpoint in [
            ...
            assert!(PolicyTransport::new(endpoint, false, "/chat/completions")
                .await
                .is_err());
        }
        assert!(
            PolicyTransport::new("http://localhost:11434", true, "/api/chat")
                .await
                .is_err()
        );
        let client = PolicyTransport::new("http://127.0.0.1:11434", true, "/api/chat")
            .await
            .unwrap();
        for (method, url) in [
            ("GET", "http://127.0.0.1:11434/api/chat"),
            ("POST", "https://elsewhere.example/api/chat"),
            ("POST", "http://127.0.0.1:11434/api/pull"),
            ("POST", "http://127.0.0.1:11434/api/chat?x=y"),
        ] {
        ...
        assert!(PolicyTransport::default()
            .send::<_, Bytes>(
                Request::builder()
                    .method("POST")
    ...
    async fn sdk_clones_cannot_send_a_second_post_after_success_or_failure() {
        for status in ["200 OK", "429 Too Many Requests"] {
            ...
            let second = tokio::time::timeout(
                ...
                sdk_clone.send::<_, Bytes>(make_request()),
            )
            .await;
