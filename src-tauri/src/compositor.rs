...
fn png(uri: &Value) -> Result<Vec<u8>, String> {
    let raw = uri.as_str().and_then(|s| s.strip_prefix("data:image/png;base64,"))
...
pub fn validate_bundle(bundle: &Value) -> Result<(), String> {
    ...
    if manifest["format"] != "com.compositor.project" || manifest["version"] != 8
    ...
    if width == 0 || height == 0 || width > 30_000 || height > 30_000 || width * height > 100_000_000 {
        return Err("Compositorのキャンバス寸法が上限を超えています".into());
    }
    Uuid::parse_str(manifest["documentID"].as_str().ok_or("document IDがありません")?).map_err(error)?;
    let layers = manifest["layers"].as_array().ok_or("レイヤーが不正です")?;
    let images = bundle["images"].as_object().ok_or("素材が不正です")?;
    if layers.is_empty() || layers.len() > 64 || images.len() > 128 || serde_json::to_vec(bundle).map_err(error)?.len() > MAX_BYTES {
        return Err("Compositor連携は最大64レイヤー・96MiBです".into());
    }
    let mut expected = std::collections::HashSet::new();
    let mut ids = std::collections::HashSet::new();
    for layer in layers {
        let id = Uuid::parse_str(layer["id"].as_str().ok_or("layer IDがありません")?).map_err(error)?;
        if !ids.insert(id) { return Err("レイヤーIDが重複しています".into()); }
        for (field, suffix) in [("imageFile", ".png"), ("maskFile", ".mask.png")] {
            if layer[field].is_null() { continue; }
            let filename = format!("{}{suffix}", id.to_string().to_uppercase());
            if layer[field].as_str() != Some(filename.as_str()) { return Err("素材名が不正です".into()); }
            png(&images.get(&filename).ok_or("素材が不足しています")?["image"])?;
            expected.insert(filename);
        }
    }
    if expected.len() != images.len() { return Err("参照されていない素材があります".into()); }
    Ok(())
}

pub async fn start(id: &str, bundle: Value) -> Result<Value, String> {
    if !cfg!(target_os = "macos") { return Err("Compositor連携には対応するMacが必要です".into()); }
    validate_bundle(&bundle)?;
    let dir = directory(id)?;
    // create_new semantics: a lost launch response never launches another editor.
    std::fs::create_dir(&dir).map_err(|_| "この編集セッションは既にあります。再接続してください")?;
    #[cfg(unix)]
    {
    ...
    write(&dir.join("connection.json"), &json!({"session": id, "token": token}))?;
    ...
    let output = tokio::process::Command::new("/usr/bin/open")
        .arg("-n").arg("-a").arg(app).arg("--args").arg("--manga-session").arg(&dir)
        .output().await.map_err(error)?;
    if !output.status.success() { return Err("Compositorを起動できませんでした".into()); }
...
pub async fn exchange(id: &str, args: Value) -> Result<Value, String> {
    ...
    let request_id;
    if op == "recover" {
        request_id = read(&pending)?["id"].as_str().ok_or("照合対象がありません")?.to_owned();
    } else {
    ...
    while !path.exists() {
        if Instant::now() >= deadline { return Err("Compositorの応答が未確定です。操作を再送せず結果を照合してください".into()); }
        sleep(Duration::from_millis(100)).await;
    }
    let response = read(&path)?;
    if response["id"].as_str() != Some(&request_id) || response["session"].as_str() != Some(id) || response["protocol"] != 1 {
    ...
    if response["ok"] != true { return Err(format!("Compositor: {}", response["error"])); }
    Ok(value.clone())
}
...
mod tests {
    ...
    async fn pending_operation_is_recovered_without_resubmission() {
        ...
        write(&dir.join("connection.json"), &json!({"session":id,"token":"a".repeat(64)})).unwrap();
        let request_id = Uuid::new_v4().to_string();
        write(&dir.join("pending.json"), &json!({"id":request_id})).unwrap();
        assert!(exchange(&id, json!({"op":"transform"})).await.unwrap_err().contains("未確定"));
        write(&dir.join(format!("{request_id}.json")), &json!({"id":request_id,"session":id,"protocol":1,"ok":true,"value":{"revision":2}})).unwrap();
        assert_eq!(exchange(&id, json!({"op":"recover"})).await.unwrap()["revision"], 2);
