...
async fn response_json(response: reqwest::Response) -> Result<Value, String> {
    ...
    response
        .json()
        .await
        .map_err(|_| "TapNowの認証設定を読み取れません".into())
}
...
async fn registration_error(response: reqwest::Response) -> String {
    ...
    match code {
        Some(code) => {
            format!("TapNowがMacアプリのOAuth登録を受け付けません (HTTP {status}, {code})")
        }
...
fn validate_metadata(resource: &Value, auth: &Value) -> Result<(), String> {
    if resource["resource"] != MCP
        || auth["issuer"] != ISSUER
        || !resource["authorization_servers"]
            .as_array()
            .is_some_and(|v| v.iter().any(|x| x == ISSUER))
    ...
    for key in [
        ...
        if url.scheme() != "https"
            || url.host_str() != Some("oauth.tapnow.ai")
            || url.username() != ""
...
async fn oauth_callback(listener: TcpListener, state: &str) -> Result<String, String> {
    let (mut stream, peer) = tokio::time::timeout(Duration::from_secs(180), listener.accept())
        .await
        .map_err(|_| "TapNow認証が時間切れです")?
        .map_err(|_| "認証の戻り先を受信できません")?;
    if !peer.ip().is_loopback() {
        return Err("認証の戻り先が不正です".into());
    }
    let mut bytes = [0u8; 4096];
    let size = tokio::time::timeout(Duration::from_secs(5), stream.read(&mut bytes))
        .await
        .map_err(|_| "認証の応答が時間切れです")?
        .map_err(|_| "認証の応答を読めません")?;
    ...
    let valid = url.path() == "/tapnow/callback"
        && params.get("state").map(String::as_str) == Some(state)
        && params
            .get("iss")
            .map(String::as_str)
            .is_none_or(|iss| iss == ISSUER);
    let code = if valid {
        params.get("code").filter(|code| !code.is_empty()).cloned()
    } else {
        None
    };
    let message = if code.is_some() {
        "TapNowの認証を受け取りました。アプリへ戻ってください。"
    } else {
        "TapNowの認証を完了できませんでした。"
    };
...
pub async fn connect(connection: &Connection) -> Result<Value, String> {
    ...
    let resource = response_json(
        http.get(RESOURCE)
            .send()
            .await
            .map_err(|_| "TapNowへ接続できません")?,
    )
    .await?;
    let auth = response_json(
        http.get(METADATA)
            .send()
            .await
            .map_err(|_| "TapNowへ接続できません")?,
    ...
    let listener = TcpListener::bind("127.0.0.1:0")
        .await
        .map_err(|_| "認証の戻り先を開けません")?;
    let redirect = format!(
        "http://127.0.0.1:{}/tapnow/callback",
        listener
            .local_addr()
            .map_err(|_| "認証の戻り先を確認できません")?
            .port()
    );
    ...
    let client_id = registration["client_id"]
        .as_str()
        .ok_or("TapNowからクライアントIDが返されません")?;
    let mut authorize = Url::parse(auth["authorization_endpoint"].as_str().unwrap())
        .map_err(|_| "認証先が不正です")?;
    authorize
        .query_pairs_mut()
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        if !opened.success() {
            return Err("ブラウザでTapNow認証を開けません".into());
        }
    }
    ...
    let token = http
        .post(auth["token_endpoint"].as_str().unwrap())
        .form(&[
...
pub fn disconnect(connection: &Connection) -> Result<(), String> {
    *connection
        .0
        .lock()
        .map_err(|_| "TapNow接続を解除できません")? = None;
    Ok(())
}
...
fn decode_mcp(body: &str, sse: bool, expected_id: i32) -> Result<Value, String> {
    if !sse {
        ...
        return if value["id"] == expected_id {
            Ok(value)
        } else {
            Err("TapNowのMCP応答IDが不正です".into())
        };
    }
...
async fn list_tools_with_bearer(bearer: &str) -> Result<Value, String> {
    ...
    for (id, method, params) in [
        (
            1,
            "initialize",
            json!({"protocolVersion":"2025-03-26", "capabilities":{}, "clientInfo":{"name":"manga-mac","version":"0.1.0"}}),
        ),
        ...
        let response = request
            ...
            .send()
            .await
            .map_err(|_| "TapNowのツール一覧を取得できません")?;
        if !response.status().is_success() {
            return Err(format!(
                "TapNowのMCP接続に失敗しました (HTTP {})",
                response.status()
            ));
        }
        ...
        let value = decode_mcp(
            &response
                .text()
                .await
                .map_err(|_| "TapNowのMCP応答を読めません")?,
            sse,
            id,
        )?;
        if value.get("error").is_some() {
            return Err("TapNowのMCPが要求を拒否しました".into());
        }
        if method == "tools/list" {
            let tools = value["result"]["tools"]
                .as_array()
                .ok_or("TapNowのツール形式が不正です")?;
...
mod tests {
    ...
    fn rejects_untrusted_metadata_and_ignores_unrelated_sse_events() {
        ...
        wrong["token_endpoint"] = json!("https://other.example/token");
