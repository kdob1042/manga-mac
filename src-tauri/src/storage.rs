...
fn valid_hash(s: &str) -> bool { s.len() == 64 && s.bytes().all(|b| b.is_ascii_hexdigit() && !b.is_ascii_uppercase()) }
fn sync_dir(dir: &Path) -> Result<()> { fs::File::open(dir).and_then(|f| f.sync_all()).map_err(err) }
...
fn put(dir: &Path, bytes: &[u8]) -> Result<String> {
    ...
    if path.exists() { verify(dir, &id)?; return Ok(id); }
    ...
    if hash(&fs::read(&temp).map_err(err)?) != id { return Err("Artifact verification failed".into()); }
    match fs::hard_link(&temp, &path) {
        Ok(()) => (),
        Err(e) if e.kind() == std::io::ErrorKind::AlreadyExists => { verify(dir, &id)?; },
...
fn verify(dir: &Path, id: &str) -> Result<Vec<u8>> {
    if !valid_hash(id) { return Err("Invalid artifact ID".into()); }
    let path = dir.join(id);
    if fs::symlink_metadata(&path).map_err(err)?.file_type().is_symlink() { return Err("Artifact symlink rejected".into()); }
    let bytes = fs::read(path).map_err(err)?;
    if hash(&bytes) != id { return Err("Artifact hash mismatch; adopted data was not changed".into()); }
    Ok(bytes)
}
fn image_field(key: &str) -> bool { matches!(key, "image" | "original" | "mask") }
fn externalize(value: &mut Value, dir: &Path) -> Result<()> {
    match value {
        Value::Array(items) => for item in items { externalize(item, dir)?; },
        Value::Object(items) => for (key, item) in items {
            if image_field(key) {
                if let Some(uri) = item.as_str().filter(|s| s.starts_with("data:image/")) {
                    ...
                    if !["data:image/png;base64", "data:image/jpeg;base64", "data:image/webp;base64"].contains(&prefix) { return Err("Unsupported image type".into()); }
                ...
                } else if item.get("artifact_id").is_some() { read_image(item, dir)?; }
            }
...
fn read_image(item: &Value, dir: &Path) -> Result<String> {
    let id = item["artifact_id"].as_str().ok_or("Invalid artifact reference")?;
    if item["hash"].as_str() != Some(id) { return Err("Artifact reference mismatch".into()); }
    let prefix = item["prefix"].as_str().ok_or("Invalid image prefix")?;
    if !["data:image/png;base64", "data:image/jpeg;base64", "data:image/webp;base64"].contains(&prefix) { return Err("Invalid image prefix".into()); }
    let bytes = verify(dir, id)?;
    if item["size"].as_u64() != Some(bytes.len() as u64) { return Err("Artifact size mismatch".into()); }
    Ok(format!("{prefix},{}", STANDARD.encode(bytes)))
}
fn hydrate(value: &mut Value, dir: &Path) -> Result<()> {
    match value {
        Value::Array(items) => for item in items { hydrate(item, dir)?; },
        Value::Object(items) => for (key, item) in items {
            if image_field(key) && item.get("artifact_id").is_some() { *item = Value::String(read_image(item, dir)?); }
            else { hydrate(item, dir)?; }
        },
        _ => (),
    }
    Ok(())
...
pub fn initialize(db: &Connection) -> Result<()> {
    db.execute_batch("PRAGMA journal_mode=WAL; PRAGMA synchronous=FULL;
      ...
      CREATE TABLE IF NOT EXISTS project_backups(hash TEXT PRIMARY KEY,data TEXT NOT NULL);").map_err(err)
}
pub fn save(db: &mut Connection, root: &Path, data: &str) -> Result<()> {
    let mut project: Value = serde_json::from_str(data).map_err(err)?;
    if !matches!(project["version"].as_u64(), Some(1 | 2)) { return Err("Unsupported project schema".into()); }
    // Commit the previous exact JSON before starting file migration.
    let previous: Option<String> = db.query_row("SELECT data FROM project WHERE id=1", [], |r| r.get(0)).optional().map_err(err)?;
    let backup = previous.as_deref().unwrap_or(data);
    db.execute("INSERT OR IGNORE INTO project_backups(hash,data) VALUES(?1,?2)", [&hash(backup.as_bytes()), backup]).map_err(err)?;
...
pub fn load(db: &Connection, root: &Path) -> Result<Option<String>> {
    let data: Option<String> = db.query_row("SELECT data FROM project WHERE id=1", [], |r| r.get(0)).optional().map_err(err)?;
    data.map(|data| {
...
mod tests {
    ...
    fn setup() -> (Connection, std::path::PathBuf) {
        let dir = std::env::temp_dir().join(format!("manga-storage-{}-{}", std::process::id(), std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_nanos()));
        fs::create_dir(&dir).unwrap();
        let db = Connection::open(dir.join("test.sqlite3")).unwrap(); initialize(&db).unwrap(); (db, dir)
    }
    fn fixture() -> Value { serde_json::from_str(include_str!("../../tests/fixtures/legacy-v1.json")).unwrap() }
    #[test]
    fn legacy_restart_and_history_roundtrip() {
        ...
        let stored: String = db.query_row("SELECT data FROM project", [], |r| r.get(0)).unwrap();
        ...
        assert_eq!(db.query_row::<String, _, _>("SELECT data FROM project_backups", [], |r| r.get(0)).unwrap(), original.to_string());
    ...
    fn corrupt_artifact_never_advances_database() {
        let (mut db, dir) = setup(); let original = fixture();
        save(&mut db, &dir, &original.to_string()).unwrap();
        let before: String = db.query_row("SELECT data FROM project", [], |r| r.get(0)).unwrap();
        let stored: Value = serde_json::from_str(&before).unwrap();
        let id = stored["panels"][0]["image"]["artifact_id"].as_str().unwrap();
        ...
        assert_eq!(db.query_row::<String, _, _>("SELECT data FROM project", [], |r| r.get(0)).unwrap(), before);
