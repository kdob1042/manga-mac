...
try{
 ...
 const payload=await page.evaluate(async({legacy,hash,size})=>{
  const {migrateProject}=await import('/src/revisions.js');const {shotFromPanel,assignMotion}=await import('/src/panel-motion.js');const {beginVideoJob,collectVideoResult,adoptVideoCandidate}=await import('/src/video.js');const {prepareLiveManga}=await import('/src/live-export.js');const {imageOf}=await import('/src/render.js');
