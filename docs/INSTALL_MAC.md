...
| クラウド作画・動画 | OpenAIまたはRunwayの手動APIキー登録・送信許可・費用枠 |
| 3D構図・撮影 | アプリ内の3Dステージ |
...
クラウド作画は「接続・人物設定」→「画像生成モデル」で **OpenAI / GPT Image 2.5 Sunburst** または **Runway / Gen-4 Image** を選び、その下でAPIキー・作品の予算枠・送信許可を登録します。登録時の接続試験や自動生成はありません。設定を閉じてコマの生成候補を作成してください。クラウドだけを使う場合、ローカルFLUXの取得は不要です。

OpenAIは人物・画風・編集元など合計8枚まで、Runway静止画は3枚まで対応します。OpenAIは1要求1 USDの仮予約（従量課金の厳密な上限ではありません）、Runwayは画面に表示するcreditsで予算を判定します。動画は「動画生成の設定」でRunwayモデル・キー・予算を登録します。

キーはアプリの起動中メモリだけに保持し、作品やバックアップ、GitHubには保存しません。終了後は再入力が必要です。OpenAIで通信が途切れた場合は自動再送せず、受信済み画像だけを回収します。応答画像そのものを失った場合は後から照会できません。Runwayは同じキーを再登録して保存済みtaskを照会できます。
