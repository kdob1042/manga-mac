...
アプリで「原稿を開く」「変更を確認」または「更新を確認」を押すと、選択した `dev` または `main` を `origin` から取得します。その後、本文と参照画像を同じcommitのGitオブジェクトから読みます。ターミナル操作は不要です。作業ツリーは読み書きせず、アプリはpullやpushを実行しません。取得に失敗した場合は古いcommitへ黙って戻らず、画面にエラーを表示します。設定ファイルを削除するとGitHub APIからの読取りに戻ります。
