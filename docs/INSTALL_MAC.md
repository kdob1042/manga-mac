...
アプリは選択した `dev` または `main` の `origin/<branch>` を確認し、本文と参照画像を同じcommitのGitオブジェクトから読みます。作業ツリーは読みません。新しい原稿を確認する前に、ローカルリポジトリで `git fetch origin dev`（または `main`）を実行してください。アプリはfetch、pull、pushを実行しません。設定ファイルを削除するとGitHub APIからの読取りに戻ります。
