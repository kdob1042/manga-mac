...
この結果を受け、既知の数値目標が未達なら応答schemaからreadyを除外する。actionとblockedを残し、不足時に操作の捏造を強制しない。保存状態の読戻しと独立した完了検査は維持する。関連Node回帰14件成功。実モデル試験は個別失敗を記録したまま、撮影済みコマへの自然言語修正も継続できるようにした。新しい実モデル結果は当該PRのActionsを参照。
