...
## 2026-09-24 コマ枠ドラッグの操作・表示安定化（#329 / PR #330）

対象: 最新dev `251db23`を起点とした実装 `4abc9c8`（ローカル検証版とtree `7d91695241d154364cb38857f70412b12474bde5`の完全一致を確認）。後続変更はこの検証記録のみ。

- 作画・コマ割り編集の通常枠内ドラッグは選択のみ。Option（Alt）で明示移動し、辺／角の形状変更と区別する。斜辺の端を隣接辺上に維持し、画面距離によるスナップ・ページ境界／最小寸法制限を共通化した。
- 旧 `PageProof.jsx`（dev `a2b082c`）へ一時的に戻した比較試験で、コマ縮小の再描画中に原稿表示領域が高さ1130pxから0pxになることを再現。新実装では同じポインター操作・画像読込遅延で表示寸法とスクロール位置が一定。
- Node.js 24: `npm test` 436件成功。`npm run build` 成功（既存のchunk警告あり）。
- Linux Chromium 153: `CHROMIUM_EXECUTABLE_PATH=<chromium> npx playwright test --workers=2` 全100件成功。誤移動防止、スナップから元位置への復帰がUndoを増やさないこと、斜辺の外周維持、明示移動、Esc、過大縮小、保存／再読込／Undo、原稿・作画・他コマの保持を確認。追加UI試験は2件、幾何試験は3件。
- Mac実機／WKWebView・DMGは未実施。今回のWeb検証を実機受入済みとはしない。対象Macでの操作確認は #266。
...
## 責務分離リファクタリング（Issue #92）

2026-09-17、dev `209e3ad` を起点。制作工程をUIから分離し、画像処理／ページ描画／出力を分けた。保存schema・UI操作・生成／採用契約・Rust/Swift/Blender実装に変更なし。

- Node: 101件成功（既存97件＋停止・再開・生成失敗保存・原文索引の回帰4件）。
- Playwright: 26件成功。初稿、局所修正、crop、PNG/CBZ、Undo、保存・再読込、画像／動画／Blender復旧を含む。Linux上のChromiumを利用し、native呼出は既存fixture。実Mac・実モデル・実Blender受入を意味しない。
- 静的確認: ESLintのno-undef／no-unused-varsによるsrcチェックで指摘なし。プロジェクトに専用lint/typecheckコマンドはない。既存依存・CI設定は変更していない。
...
## 2026-09-24: 作画ページの配置統一（Issue #299）

- 作画画面の固定2×2表示を廃し、既存 `pagePNG` による採用済み枠・画像・文字の仮ページを表示。コマ選択はページ上、局所修正範囲は元画像上の座標で指定する。完成ページと同じrendererを使用。
- Node 416件とVite build、UIテストの構文列挙（88件）成功。1コマ・6コマ・元画像範囲・完成ページ一致のChromium UIテストを追加したが、この環境はChromium未導入で、取得先のzip応答が不正だったため実行できていない。
- Mac配布ビルド、実機表示、配信ビューアでの目視照合は未実施。実機受入は #266 で行う。

## 2026-09-24: ネームから作画への継続状態（Issue #313）

- 任意のpanel.continuityを共有schemaで検査し、同一場面の既出コマと登場人物だけを参照できるようにした。取込時にIDを変換し、候補・採用・保存・作画promptへ渡す。旧v2ファイルは受理する。
- 同じ場面の採用済み前コマ画像は人物正本・画風参照を確保した残りのモデル参照枠だけに入れる。画像hashは生成Job入力に固定し、前コマ差替え後の自動採用を拒否する。
- 最終版でNode全体432件、Vite build成功。追加回帰では新旧v2、保存後prompt、参照元変更、バッチ、画像間QAの同意と失効、長すぎる指示のJob予約前拒否、明示状態の優先・再起動・解除を確認。
- 画像間Continuity QAは採用済み2コマと送信許可で指摘を保存し、原画像変更時に古い回答を拒否する。実モデルでの指摘品質は未確認。
- 手修正用の状態固定/解除UIを追加した。Mac実機の推論と衣装・小道具・感情の画質比較、UI実機操作は未実施。残件は #315。
