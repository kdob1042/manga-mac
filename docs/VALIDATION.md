...
- 最終版でNode全体431件、Vite build成功。追加回帰では新旧v2、保存後prompt、参照元変更、バッチ、画像間QAの同意と失効、長すぎる指示のJob予約前拒否を確認。
- 画像間Continuity QAは採用済み2コマと送信許可で指摘を保存し、原画像変更時に古い回答を拒否する。実モデルでの指摘品質は未確認。
- Mac実機の推論と衣装・小道具・感情の画質比較、手修正用の状態lock/override UIは未実施。Issue #313を開いたまま残件を追う。
