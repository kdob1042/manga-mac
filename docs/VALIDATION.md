...
# 2026-09-26: ページネームの人物参照自動取込み（#339）
...
- 追加確認: 旧ボタン名を現在の「GitHubのネームを開く」に追従し、人物画像の保存完了を待ってから次のネームを選ぶよう試験を補正。関連15件成功後、全体102件を再実行し全件成功。製品の待機・検証条件は緩和していない。
- Web production build: 成功。Mac aarch64のapp bundle build: 成功。Rust targetは正本のsrc-tauri/targetを使用した。
- Mac実機: 修正版アプリのGitHubページ取得から、同じcommitの人物参照3件を自動取得・採用し、手動の人物登録なしで4コマのFLUX.2 klein 4B作画を開始。完成ページの画質確認は別途記録する。
