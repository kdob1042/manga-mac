...
原稿の更新確認は取込み画面の「変更を確認」または原稿工程の「更新を確認」の明示操作のみ。起動時・定期の自動確認は行わない。通常はGitHub APIを使用する。アプリデータの `local-source.json` にrepoと絶対pathを明示した作品だけは、原稿一覧の読み込み・変更確認時にアプリが選択ブランチを `origin` からfetchし、ローカルGitの `origin/dev` または `origin/main` をcommitへ解決する。取得に失敗した場合は保存済みsnapshotと漫画を維持してエラーを表示し、古い追跡refで更新なしと判定しない。`git cat-file` で本文と画像を同じcommitから読み取り、作業ツリーには依存しない。アプリはpull・pushを実行しない。場面／設定／参照画像の追加・変更・削除とmanifest変更の概要を一時表示し、「取り込む」で同一commitの不変SourceSnapshotをactiveにする。未取込みの候補はメモリ内だけに保持し、再起動・対象作品／話変更で破棄する。確認失敗・保存失敗では既存正本と漫画を維持する。旧snapshotは既存漫画の出所・履歴として保持し、最新正本はactiveの1版とする。漫画への反映状態のdiffとは別処理である。本文・設定・参照画像に差分がなくても、commitが変わればネーム等の成果物を含む取得版更新として確認・明示採用できる。採用前に選択branchのHEADを再確認し、移動していれば候補を破棄して再確認を求める。ローカル経路では設定repoとoriginのGitHub URLの一致を検証し、取得経路・確認済みcommit・採用済みcommitを画面で区別する。作業ツリーの未commit変更は取込対象ではない。
...
標準導線は採用済み作画のコマ上の「このコマから動画を生成」。単発は設定済みの動画接続・作品予算内で、対象画像の不変版、コマ固有の原文範囲、人物、ネームの演出意図から指示を作り、同じ画面から送信する。演出LLMが視覚入力に対応する場合だけ画像も案の作成に渡す。LLMが未接続なら画面内容を認識したと装わず、構図保持と控えめな動きの定型指示を使う。利用者はコマの詳細から送信前の指示を任意で上書きできる。動画生成モデルには採用作画PNGと最終指示だけを送り、原稿全文をそのまま転送しない。

画像と選択モデルの縦横比が一致しなければ有料送信前に止める。接続が未設定なら設定画面へ案内する。選択コマの標準導線は同じ漫画画面で件数・モデル・最大予約creditsを示して確認し、全コマの案とmanifestを検証してから保存済みバッチを逐次実行する。旧動画画面の個別レシピ編集は詳細導線として維持する。どちらも既存Jobの保存→送信、結果不明の再POST禁止、候補再生・採用・Undoを共有し、生成直後の自動採用やコマへの自動割当は行わない。
...
レイアウト編集は原作・作画・3D構図・動画と独立。作画ページのコマ枠を直接ドラッグし、辺で幅・高さ、角で形、枠内で位置を調整できる。同じ枠編集はコマ割り工程でも使える。ポインター移動中はReact内の一時案として枠線を追従させ、pointerupで一回だけ保存・ページを再描画する。取消・Esc・capture消失は破棄。専用layoutHistory/layoutRedoはレイアウトだけ最大100操作保持し、画像や生成Jobを巻き戻さない。
...
作画工程のページも採用済みlayout・crop・配置済みの文字・枠破りをpagePNGで表示する。旧形式の未配置キャプションfallbackは作画ページに描かず、ページ外の原稿欄と「文字配置待ち」で示す。保存済みのキャプションや吹き出しは仕上げ・出力のページ描画と揃え、仮の白帯をページに足さない。未作画は同じレンダラーの仮表示を使う。ページ上の枠でコマを選択・直接変形する。画像の中身の部分修正範囲だけは選択した元画像で指定して原画像座標を維持し、枠サイズ調整と区別して案内する。選択輪郭とドラッグ取っ手は画面操作用の重ね表示で出力に含めない。作画工程に固定2×2の代替表示を置かない。
...
漫画制作の入力はネームと固定人物ID別の参照画像。原稿の別途取込みは不要で、原稿編集はstory-libraryが担当する。`name-plan/v2` の `source.kind=embedded` は作成時の場面原文、対象atom、人物ID/名前、話ID、1〜999999のネーム番号を含む。番号別に前半・後半など任意範囲を追加し、既存番号の明示更新だけがその番号のページを置換する。別番号のコマ・作画・手動配置を残す。

最新原稿・設定・人物画像のhash照合、原稿更新のたびの保存済みネーム検査は行わない。原文と参照位置の検査は内包された原文に対して行う。画像は現在登録された固定人物IDから解決し、足りない場合は該当コマの作画時にIDを示す。他のコマは継続できる。画像キャッシュは導入しない。内部の不変原文の保存、破損検知、Jobの結果回収、保存CASは維持する。

採用ネームは選択中の `namePlan` と他番号の `otherNamePlans` に重複なく分ける。全番号のコマは既存panels/layoutへ保存し、番号選択は編集・制作対象のみを切り替える。保存・読戻し・Undoは全番号を含む。原文を含まない旧形式と旧原稿制作経路は既存作品の互換用。以下の旧経路の原稿同期条件は原文入りネームへ適用しない。操作は[ネーム取込](NAME_PLAN_IMPORT.md)を参照。
...
- #313の作画継続状態: name-plan/v2のpanel.continuityは任意。旧v2は受理する。人物はコマの登場人物、previousPanelIdは同じ場面の既出コマに限り、取込時にアプリ内IDへ変換して候補・採用・保存へ運ぶ。画像要求は短い視覚状態を作画指示へ加え、モデルの参照枠に空きがあれば採用済み前コマを補助画像に使う。人物正本・画風参照を優先し、前コマの画像hashをJob入力に固定する。バッチは各コマ完了後の採用画像を次の入力にできる。前コマの画像変更後に旧候補を自動採用しない。画像間continuity QAは送信許可後に採用済み2コマを画像対応AIへ渡し、注意・警告だけを保存する。コマごとの明示固定はpanel.continuityOverrideを優先し、状態・参照元の編集、解除、通常Undo、再起動を扱う。既存画像を直ちに変更せず、次の作画要求から効く。実モデルの品質比較は後続実施とする。

保存は既存project.namePlanとotherNamePlans・panels・sourceApplication・Job・draft checkpointに加算。新規ネームは作成時の原文を内包し、最新原稿・人物設定・画像版との一致を採用条件にしない。APIキーは保存しない。実行中／応答不明要求の自動再送、モデルの自動取得、クラウドfallback、公開転送はしない。
...
GitHubの選択ページ取得では、同じ固定commitの `work.json` に宣言された使用人物の画像を既存native画像検証境界から取得し、候補採用時に人物参照へ反映する（#339）。話の携帯用characterIdは変更せず、制作投影でrepo・workId・原稿characterIdが一致する既存人物IDへ対応付ける。名前だけで別作品・別repoへ結び付けない。必要画像が未宣言・欠損・不正なら取得を停止し、既存作品を変更しない。v3作画で参照不足の場合も無言でスキップせず、要求・Job保存前に理由を表示する。
