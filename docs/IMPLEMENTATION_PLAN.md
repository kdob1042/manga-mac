...
Live の `clip` / `frame` / 各コマ `artRect` はホーム枠のまま。はみ出しは `pagePNG` 系のページ画像にだけ焼く。contracts と Rust clip 検証は変えない。コマ動画の再生領域はホーム枠で、静止画と同じ cover＋polygon mask。拡大表示だけクロップせず動画全体を見せる。
