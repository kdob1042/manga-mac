...
レイアウト編集は原作・作画・3D構図・動画と独立。作画ページのコマ枠を直接ドラッグし、辺で幅・高さ、角で形を調整できる。枠内の通常ドラッグは選択だけにし、移動はOption（Alt）＋ドラッグに限定する。辺は隣接辺との交点を動かし、斜辺でも接していた水平・垂直の外周を保つ。ページ境界・他コマの端・元の端へ画面上6px以内でスナップし、ガイドを表示する。Shiftでスナップを一時解除できる。境界越え・反転・潰れる操作は有効な範囲へ制限し、元々小さい枠を拡げずに最小表示幅／高さ24px・辺長16pxを保つ。同じ枠編集はコマ割り工程でも使える。ポインター移動中はReact内の一時案として枠線を追従させ、pointerupで一回だけ保存・ページを再描画する。取消・Esc・capture消失は破棄。専用layoutHistory/layoutRedoはレイアウトだけ最大100操作保持し、画像や生成Jobを巻き戻さない。
...
作画工程のページも採用済みlayout・crop・配置済みの文字・枠破りをpagePNGで表示する。旧形式の未配置キャプションfallbackは作画ページに描かず、ページ外の原稿欄と「文字配置待ち」で示す。保存済みのキャプションや吹き出しは仕上げ・出力のページ描画と揃え、仮の白帯をページに足さない。未作画は同じレンダラーの仮表示を使う。ページ上の枠でコマを選択・直接変形する。画像の中身の部分修正範囲だけは選択した元画像で指定して原画像座標を維持し、枠サイズ調整と区別して案内する。選択輪郭とドラッグ取っ手は画面操作用の重ね表示で出力に含めない。作画工程に固定2×2の代替表示を置かない。非同期再描画中や画像読込前もページ表示の縦横比1600:2260と幅を維持し、待機・失敗表示でスクロール領域を潰さない。古い版の画像を現在版として表示しない。
