...
静止リグへの初期ポーズ窓口は、固定checkpoint内のローカルAction assetをBlender既存`Pose.apply_pose_from_action`で評価する。単一slotのボーンtransformだけを許可し、アニメーション/NLA/driver/constraint付き・リンク/override・複数Scene共有の対象は拒否する。対象Armatureの共有選択状態を分離し、新checkpointへ保存・再読込してから確定する。複雑なリグや外部Pose Library横断取込の完成とは扱わない。
