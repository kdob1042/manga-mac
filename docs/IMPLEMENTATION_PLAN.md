...
### レイアウト変更・後続reflow・確定境界（#77 / #82）

レイアウト変更は、**局所編集**、**変更地点以降の決定的reflow**、**確定境界以降のAI再計画**を分けて扱う。

局所編集は現在ページのpanel割当を変えず、枠の大きさ・四隅・斜め境界・crop・同一ページ内の配置だけを変更する。前後ページのpanel割当は変更せず、reflowを起動しない。作画、原文、panel ID、Blender、動画を再生成せず、枠やcrop等のslot容量を変えない変更はページ境界へ波及させない。

枠数やページ境界を変える場合は、最初の変更地点より前を完全保持し、その地点から作品末尾までpanelの原文順を唯一の順序として決定的にreflowする。容量不足なら後続へpushし、余剰容量があれば前へpullして末尾の不要枠／ページを詰める。途中の任意ページだけを固定する`page.locked`や島状lockは持たず、reflowだけでLLM/Jev/画像生成/Blender/動画を呼ばない。

作品には先頭から連続した確定範囲を1つだけ持ち、`confirmedThroughPanelId`で末尾をstableに保持する。確定済みprefixのgeometry、panel割当、作画、文字、crop、原文参照はreflow・AI再計画・Undoで変更しない。確定範囲を編集する場合は、黙って解除せず境界を前へ戻す明示操作を要求する。飛び石の確定状態は作らない。
